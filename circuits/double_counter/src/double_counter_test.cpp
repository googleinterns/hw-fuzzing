...
void DoubleCounterTest::OpenSimulationFile() {
    ...
    //// size format.
    //std::string line = "";
    //while (getline(*input_file_stream_, line)) {
        //if (line.length() > 1) {
            //CloseSimulationFile();
            //exit(0);
        //}
    ...
    //input_file_stream_->clear();                  // clear fail and eof bits
    //input_file_stream_->seekg(0, std::ios::beg);  // back to the start!
}
...
uint32_t DoubleCounterTest::ReadSimulationInput(
    ...
    if (!input_file_stream_->eof()) {
        ...
        //// Read file line by line, returning first byte on the line
        //std::string line = "";
        //std::getline(*input_file_stream_, line);
        //buffer = (uint8_t*) line.c_str();
        //return 1;
    }
    return 0;
}
    ...
    while (!Verilated::gotFinish() && num_bytes_read_from_input_file_) {
        ...
        if ((main_time_ % 2) == 0) {
            ...
            if (num_bytes_read_from_input_file_) {
                ...
                std::cout << std::hex << unsigned(select_);
                std::cout << std::endl;
...
int main(int argc, char** argv, char** env) {
    ...
    DoubleCounterTest* tb = new DoubleCounterTest(argv[1]);
